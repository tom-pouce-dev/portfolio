<?php
	...
	$articles = $mysql->prepare('SELECT * FROM contenus JOIN users ON contenus.created_by = users.id');
	$articles->execute();

    $data = $articles->fetch();
    
	$aside = $mysql->prepare('SELECT contenus.id, title, imgTexte, categorie FROM contenus WHERE categorie = :a ORDER BY id DESC LIMIT 2');
	...
	<main class="wrapper">
		...
		<section class="container gutters">
			<article class="l-col-8">
				...
				<figure>
					<img src="<?php echo $data['imgTexte'] ?>" alt="[antoine barilt fablab-presentation][<?php echo $data['title']; ?>]">
				</figure>
