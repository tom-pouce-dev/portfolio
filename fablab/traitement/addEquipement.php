...
if( empty($_POST) ){
	...
	header('Location: ../back/addEquipement.php');
	exit();
...
$error = false;
$errorUrl = '../back/addEquipement.php?';
...
if(empty($_POST['machine'])){
	...
	$errorUrl = $errorUrl.'&title='.$_POST['machine'];
}

//la fiche machine est vide
if(empty($_POST['fiche'])){
	$error = true;
	$errorUrl = $errorUrl.'&ficheEmpty';
}else{
	$errorUrl = $errorUrl.'&resume='.urlencode($_POST['fiche']);
}
...
if($error){
	...
	if($_FILES['visuel']['size'] > 0){
		...
		$nom = 'img/3dhubs/machine-'.$id.'-'.$nbre.'.'.$extension_minuscule;
	...
	header('Location: ../back/equipement.php?id='.$id.'&equipementAdded');
